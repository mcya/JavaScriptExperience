...
节点操作：
  ...
  添加节点： oDiv.appendChild(...);
  删除节点： oDiv.removeChild(oTr);

获取： id, className, tagName, name
   父节点: parentNode,
   子节点: childNodes
   兄弟节点： previousElementSibling, previousSibling
...
兼容：
    获取 css 对象：  oDiv.currentStyle || getComputedStyle(oDiv)

    获取下一个元素节点： 通过 nextSibling 来封装一个函数
	getElementsByClassName 的兼容写法


应用： 进度条、手风琴、tab切换、表格创建、网页换肤
